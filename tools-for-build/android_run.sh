android_run() {
    ...
    (adb shell "LD_LIBRARY_PATH=\"$SBCL_ANDROID_TARGET_LOCATION/output/android-libs\" \"$SBCL_ANDROID_TARGET_LOCATION/tools-for-build/temp.out\" 2>/dev/null") | sed 's/\r//g'
...
android_run_for_exit_code() {
    ...
    (adb shell "echo input | LD_LIBRARY_PATH=\"$SBCL_ANDROID_TARGET_LOCATION/output/android-libs\" \"$SBCL_ANDROID_TARGET_LOCATION/tools-for-build/$1\" > /dev/null 2>&1 ; echo \"\$?\"" 2>/dev/null) | sed 's/\r//g'
